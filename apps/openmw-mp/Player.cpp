...
{
    LOG_MESSAGE_SIMPLE(MWMPLog::LOG_INFO, "Deleting player with guid %lu", guid.g);

    if (players[guid] != 0)
    {
        CellController::get()->deletePlayer(players[guid]);

        LOG_APPEND(MWMPLog::LOG_INFO, "- Emptying slot %i", players[guid]->getId());
...
{
    LOG_MESSAGE_SIMPLE(MWMPLog::LOG_INFO, "Creating new player with guid %lu", guid.g);
    ...
    for (unsigned int i = 0; i < mwmp::Networking::get().maxConnections(); i++)
    {
        if (slots[i] == 0)
        {
            LOG_APPEND(MWMPLog::LOG_INFO, "- Storing in slot %i", i);
